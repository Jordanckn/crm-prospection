...
const server = new McpServer(
  { name: `crm-${brandName.toLowerCase().replace(/[^a-z0-9]+/g, "-")}`, version: "1.2.0" },
  {
    instructions: [
      ...
      "Dans l'espace Epiderme AI, le pays du contact est obligatoire et doit valoir exactement France ou Israël.",
      "Règle d'identité absolue : France utilise Epiderme AI (avec e final) et contact@epiderme-ai.com ; Israël utilise Epiderm AI (sans e final) et contact@epiderm-ai.com.",
      "Ne choisis jamais un expéditeur d'après la langue, le template ou ton intuition : vérifie et corrige d'abord le champ pays du contact. Le CRM applique ensuite le routage d'email automatiquement.",
...
server.registerTool("crm_create_contact", {
  ...
  inputSchema: {
    ...
    interet: z.enum(["", "Intéressé", "Non intéressé"]).optional(),
    pays: z.enum(["France", "Israël"]).default("France").describe("Détermine l'identité d'envoi : France = Epiderme AI ; Israël = Epiderm AI"), secteur_activite: z.string().optional(),
...
server.registerTool("crm_update_contact", {
  ...
  inputSchema: {
    ...
    interet: z.enum(["", "Intéressé", "Non intéressé"]).optional(),
    assigned_to: z.string().uuid().optional(), pays: z.enum(["France", "Israël"]).optional().describe("France = Epiderme AI ; Israël = Epiderm AI"), secteur_activite: z.string().optional(),
    adresse: z.string().optional(), ville: z.string().optional(), code_postal: z.string().optional(),
    notes_entreprise: z.string().optional(), site_web: z.string().url().optional(),
    instagram: z.string().max(500).optional().describe("URL ou identifiant Instagram"),
    facebook: z.string().max(500).optional().describe("URL ou identifiant Facebook"),
...
server.registerTool("crm_record_followup", {
  ...
  inputSchema: {
    ...
    contact: z.object({
      ...
      statut: z.enum(["Nouveau", "En cours", "Converti", "Perdu"]).optional(),
      interet: z.enum(["", "Intéressé", "Non intéressé"]).optional(),
      pays: z.enum(["France", "Israël"]).optional().describe("Obligatoire pour un nouveau contact : France = Epiderme AI ; Israël = Epiderm AI"), secteur_activite: z.string().optional(), notes_entreprise: z.string().optional(), site_web: z.string().optional(),
